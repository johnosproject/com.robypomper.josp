...
public class StructureController {
    ...
    public ResponseEntity<JOSPStructHtml> jsonObjectStructure(@PathVariable("obj_id") String objId) {
    ...
    public String htmlObjectStructure(@PathVariable("obj_id") String objId) {
        JOSPStructHtml structHtml = jsonObjectStructure(objId).getBody();
        if (structHtml == null)
        ...
        try {
            return HTMLUtils.toHTMLFormattedJSON(structHtml,
                    String.format("%s Object's Structure", jslService.getObj(objId).getName()),
    ...
    public ResponseEntity<JOSPComponentHtml> jsonObjectComponent(@PathVariable("obj_id") String objId,
        ...
        JSLComponent comp = compPath.equals("-") ? obj.getStructure() : jslService.getComp(objId, compPath, JSLComponent.class);
    ...
    public String htmlObjectComponent(@PathVariable("obj_id") String objId,
                                      @PathVariable("comp_path") String compPath) {
        JOSPComponentHtml compHtml = jsonObjectComponent(objId, compPath).getBody();
        if (compHtml == null)
