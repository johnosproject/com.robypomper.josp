...
public class MngmAPIsUsrs {
    ...
    @GetMapping(path = APIJCPAPIMngm.FULL_PATH_MNGM_USRS)
    public ResponseEntity<JCPCloudStatus.JCPUsers> getJCPAPIs_UsrsReq() {
        checkAdmin();

        try {
            JSL current = jslService.getHttp(httpSession);
            JCPClient_Service jcpClient = current.getJCPClient();
            JCPCloudStatus.JCPUsers jcpAPIsResponse = jcpClient.execReq(Verb.GET, APIMngr.FULL_PATH_MNGM_USRS, JCPCloudStatus.JCPUsers.class,jcpClient.isSecured());
