...
public class MngmAPIsGWsController {
    ...
    public ResponseEntity<List<JCPAPIsStatus.GWs>> getJCPAPIs_GWsReq() {
        checkAdmin();

        try {
            JSL current = jslService.getHttp(httpSession);
            JCPClient_Service jcpClient = current.getJCPClient();
            List<JCPAPIsStatus.GWs> jcpAPIsResponse = jcpClient.execReq(Verb.GET, APIMngr.FULL_PATH_MNGM_GW, List.class, jcpClient.isSecured());
