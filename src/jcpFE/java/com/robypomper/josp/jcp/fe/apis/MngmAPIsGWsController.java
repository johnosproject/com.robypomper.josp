...
public class MngmAPIsGWsController {
    ...
    @GetMapping(path = APIJCPAPIMngm.FULL_PATH_MNGM_GWS)
    public ResponseEntity<List<JCPCloudStatus.JCPGWs>> getJCPAPIs_GWsReq() {
        checkAdmin();

        try {
            JSL current = jslService.getHttp(httpSession);
            JCPClient_Service jcpClient = current.getJCPClient();
            List<JCPCloudStatus.JCPGWs> jcpAPIsResponse = jcpClient.execReq(Verb.GET, APIMngr.FULL_PATH_MNGM_GW, List.class, jcpClient.isSecured());
