...
public class MngmAPIsObjs {
    ...
    public ResponseEntity<JCPAPIsStatus.Objects> getJCPAPIs_ObjsReq() {
        checkAdmin();

        try {
            JSL current = jslService.getHttp(httpSession);
            JCPClient_Service jcpClient = current.getJCPClient();
            JCPAPIsStatus.Objects jcpAPIsResponse = jcpClient.execReq(Verb.GET, APIMngr.FULL_PATH_MNGM_OBJS, JCPAPIsStatus.Objects.class, jcpClient.isSecured());
