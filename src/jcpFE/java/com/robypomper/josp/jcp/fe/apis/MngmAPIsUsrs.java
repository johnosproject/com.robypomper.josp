...
public class MngmAPIsUsrs {
    ...
    public ResponseEntity<JCPAPIsStatus.Users> getJCPAPIs_UsrsReq() {
        checkAdmin();

        try {
            JSL current = jslService.getHttp(httpSession);
            JCPClient_Service jcpClient = current.getJCPClient();
            JCPAPIsStatus.Users jcpAPIsResponse = jcpClient.execReq(Verb.GET, APIMngr.FULL_PATH_MNGM_USRS, JCPAPIsStatus.Users.class, jcpClient.isSecured());
