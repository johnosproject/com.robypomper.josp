...
public class JOSPPerm {
    ...
    public static String logPermissions(List<JOSPPerm> permissions) {
        StringBuilder str = new StringBuilder();
        str.append("  +--------------------+----------------------+----------------------+---------------------------+-------------------------+\n");
        str.append("  | ObjID              | SrvId                | UsrId                | Perm. and Connection Type | Updated At              +\n");
        str.append("  +--------------------+----------------------+----------------------+---------------------------+-------------------------+\n");
        for (JOSPPerm p : permissions)
            str.append(String.format("  | %-18s | %-20s | %-20s | %-10s, %-13s | %-23s |\n", p.getObjId(), p.getSrvId(), p.getUsrId(), p.getPermType(), p.getConnType(), p.getUpdatedAtStr()));
        str.append("  +--------------------+----------------------+----------------------+---------------------------+-------------------------+\n");
        return str.toString();
    }
